...
def run_transcribe(state: PipelineState, language: str, force_whisper: bool) -> bool:
    ...
    expected_path = str(UPLOAD_DIR / f"{state.video_id}_transcript.json")
    ...
    try:
        ...
        transcript_path = str(UPLOAD_DIR / f"{state.video_id}_transcript.json")
...
def run_select(state: PipelineState) -> bool:
    ...
    expected_path = str(UPLOAD_DIR / f"{state.video_id}_clips.json")
    if Path(expected_path).exists():
